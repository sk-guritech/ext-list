...
def test():
    ...
    assert ext_list_1.greater('age', 30) == [{'name': 'Charlie', 'age': 35}]
    ...
    assert ext_list_2.greater(Person.age, 30) == [charlie]
    assert ext_list_2.greater(Person.get_age_n_years_ago, 25, 5) == [charlie]


def test_reference_getset_descriptor():
    ext_list_1 = ExtList([1, 2, 3])

    assert ext_list_1.greater(int.real, 2) == [3]


def test_reference_method_descriptor():
    ext_list_1 = ExtList([1, 2, 3])

    assert ext_list_1.greater(int.bit_length, 1) == [2, 3]
