...
def test():
    ...
    assert ext_list_1.less('age', 30) == [{'name': 'Alice', 'age': 25}]
    ...
    assert ext_list_2.less(Person.age, 30) == [alice]
    assert ext_list_2.less(Person.get_age_n_years_ago, 25, 5) == [alice]


def test_reference_getset_descriptor():
    ext_list_1 = ExtList([1, 2, 3])

    assert ext_list_1.less(int.real, 2) == [1]


def test_reference_method_descriptor():
    ext_list_1 = ExtList([1, 2, 3])

    assert ext_list_1.less(int.bit_length, 2) == [1]
